...
public class ClosestBrickDetector extends Codelet {
	...
	private MemoryObject innerSenseMO;
        ...
        private List<Thing> knownLastWall;

	public ClosestBrickDetector() {
            this.knownLastWall = new ArrayList<>();
	}


	@Override
	public void accessMemoryObjects() {
		this.knownMO=(MemoryObject)this.getInput("KNOWN_BRICKS");
		   ...
		   if(known.size() != 0){
			//Iterate over objects in vision, looking for the closest jewel
				...
				String objectName=t.getName();
                                ...
                                boolean test =  knownLastWall.contains(t);
                                                                
				if(objectName.contains("Brick") ){ //Then, it is an jewel
                                        if(closest_brick == null){    
                                                closest_brick = t;
					}
                                        else {
						double Dnew = calculateDistance(t.getX1(), t.getY1(), cis.position.getX(), cis.position.getY());
                                                double Dclosest= calculateDistance(closest_brick.getX1(), closest_brick.getY1(), cis.position.getX(), cis.position.getY());
						if(Dnew<Dclosest){
                                                        closest_brick = t;
						}
					}
                                        
                                        //knownLastWall.add(closest_brick);
				}
			}
